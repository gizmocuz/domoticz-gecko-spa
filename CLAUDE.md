...
- **Shutdown ordering.** `GeckoBridge._thread_main` finally block runs the proven asyncio teardown sequence: cancel all pending tasks → `loop.run_until_complete(asyncio.sleep(0))` so cancellations propagate before the gather → `loop.shutdown_asyncgens()` → `loop.shutdown_default_executor(timeout=3.0)` → defensive `_proactor.close()` (no-op under the forced selector policy on Windows) → `loop.close()`. The asyncio-supervised executor drain is what keeps a worker blocked inside a geckolib I/O call from stalling the join. `stop()` joins the bridge thread with a 10s timeout to fit inside Domoticz's own `PythonThreadCount()` grace window.
- **Executor lifecycle.** We install our own `ThreadPoolExecutor` as the loop's default so `shutdown_default_executor()` can drain it with a bounded timeout. The raw `executor.shutdown(wait=True, cancel_futures=True)` was prone to blocking forever on a worker stuck inside a blocking geckolib socket call.
